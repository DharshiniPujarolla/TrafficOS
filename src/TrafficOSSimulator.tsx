...
const ALGORITHM_OPTIONS = [
  { id: 'FCFS', name: 'First-Come, First-Served (FCFS)', type: 'Non-Preemptive', description: 'Non-preemptive dispatch by earliest arrival timestamp across all ready queues.' },
  { id: 'SJF', name: 'Shortest Job First (SJF)', type: 'Non-Preemptive', description: 'Non-preemptive dispatch selecting the ready queue whose head process has the shortest next CPU burst.' },
...
// Bounded-buffer capacity per lane — used for the Memory Management /
// Producer-Consumer model. A lane's ready queue is a fixed-size buffer;
// addVehicle() is the producer, the dispatch/completion cycle is the consumer.
const LANE_CAPACITY = 6;

// --- Conflict Graph (DSA: Graph) ---------------------------------------
// The intersection is modeled as an undirected graph where an edge means
// "these two lanes physically conflict and can never be granted right-of-way
// simultaneously" (i.e. two perpendicular approaches). Opposite lanes
// (NORTH-SOUTH, EAST-WEST) do not conflict. This is the same conflict-graph
// abstraction used in real traffic-signal phase design.
const LANE_CONFLICT_GRAPH = {
  NORTH: ['EAST', 'WEST'],
  EAST: ['NORTH', 'SOUTH'],
  SOUTH: ['EAST', 'WEST'],
  WEST: ['NORTH', 'SOUTH']
...
// Greedy graph coloring over the conflict graph: lanes sharing a color are
// pairwise non-conflicting and could, in principle, be granted a green phase
// together. Computed generically (not hardcoded) via the classic
// greedy-coloring algorithm — visit each node, assign the smallest color
// not already used by an adjacent node.
const computeSignalPhases = (graph) => {
  const nodes = Object.keys(graph);
  const colorOf = {};

  nodes.forEach((node) => {
    const usedByNeighbors = new Set(
      (graph[node] || []).map((n) => colorOf[n]).filter((c) => c !== undefined)
    );
    let color = 0;
    while (usedByNeighbors.has(color)) color += 1;
    colorOf[node] = color;
  });
  ...
  nodes.forEach((node) => {
    ...
    if (!phases[c]) phases[c] = [];
    phases[c].push(node);
  });

  return { colorOf, phases };
};
...
const SequentialQueueVisualizer = ({ laneData, selectedAlgo }) => {
  const sortKey = selectedAlgo === 'FCFS' ? 'arrivalTime' : 'burstTime';
  const label = selectedAlgo === 'FCFS'
    ? 'FCFS Ready Queue (Sorted by Arrival Time)'
    : 'SJF Ready Queue (Sorted by Burst Time)';
  ...
  return (
    <div className="bg-white border border-[#D1D5DB] rounded-2xl p-4 shadow-sm text-xs font-mono text-[#111827] space-y-3">
      <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-2">
        <h3 className="font-bold text-xs uppercase tracking-wider text-[#0369A1]">
          {label}
        </h3>
        <span className="text-[10px] text-[#6B7280]">Non-Preemptive</span>
      </div>
        ...
        {allQueuedCars.length === 0 ? (
          ...
          allQueuedCars.map((car, idx) => (
            <div
              ...
              className={`p-2 rounded-xl border flex items-center justify-between ${
                idx === 0
                  ? 'bg-[#F0F9FF] border-[#0369A1] font-bold text-[#0369A1]'
                  : 'bg-[#F8F9FA] border-[#E5E7EB] text-[#374151]'
              }`}
            >
              <div className="flex items-center space-x-2">
                <span className="text-[10px] font-bold px-1.5 py-0.5 bg-white border rounded">
                  {selectedAlgo === 'FCFS' ? `t=${car.arrivalTime}s` : `${car.burstTime}s`}
                </span>
                <span className="text-[11px] font-sans">{car.id} ({car.lane})</span>
              </div>
              <div className="text-[10px] text-[#6B7280]">
...
const ConflictGraphPanel = ({ activeLane }) => {
  const { colorOf, phases } = useMemo(() => computeSignalPhases(LANE_CONFLICT_GRAPH), []);
  ...
  const phaseColors = ['#0369A1', '#C2410C'];
  const drawnEdges = new Set();
  const edgeList = [];
  Object.entries(LANE_CONFLICT_GRAPH).forEach(([lane, neighbors]) => {
    neighbors.forEach((n) => {
      const key = [lane, n].sort().join('-');
      if (!drawnEdges.has(key)) {
        drawnEdges.add(key);
        edgeList.push([lane, n]);
      }
    });
  });
  ...
  return (
    <div className="bg-white border border-[#D1D5DB] rounded-2xl p-4 shadow-sm text-xs font-mono text-[#111827] space-y-3">
      <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-2">
        ...
        <span className="text-[9px] text-[#6B7280]">Greedy Graph Coloring</span>
      </div>

      <div className="flex justify-center">
        <svg viewBox="0 0 280 200" className="w-full max-w-[220px]">
          {edgeList.map(([a, b]) => {
            ...
            return (
              <line
                key={`${a}-${b}`}
                x1={p1.x + 20} y1={p1.y + 20}
                x2={p2.x + 20} y2={p2.y + 20}
                stroke="#DC2626"
                strokeWidth={1.5}
                strokeDasharray="3,3"
              />
          ...
          {Object.entries(positions).map(([lane, pos]) => (
            <g key={lane}>
              <circle
                cx={pos.x + 20} cy={pos.y + 20} r="20"
                fill={lane === activeLane ? '#FEF9C3' : '#F8FAFC'}
                stroke={phaseColors[colorOf[lane]] || '#64748B'}
                strokeWidth={lane === activeLane ? 3 : 2}
              />
              <text x={pos.x + 20} y={pos.y + 24} fontSize="9" fontWeight="bold" textAnchor="middle" fill="#111827">
                {lane.slice(0, 1)}
              </text>
            </g>
          ))}
      ...
      <div className="grid grid-cols-2 gap-2">
        {Object.entries(phases).map(([color, lanes]) => (
          <div key={color} className="bg-[#F9FAFB] p-2 rounded-lg border border-[#E5E7EB]">
            <div className="text-[9px] text-[#6B7280]">PHASE GROUP {parseInt(color, 10) + 1}</div>
            <div className="font-bold mt-0.5" style={{ color: phaseColors[color] || '#64748B' }}>
              {lanes.join(' + ')}
            </div>
          </div>
        ))}
      ...
      <p className="text-[10px] text-[#6B7280] leading-relaxed font-sans">
        Red dashed edges = physical conflicts (perpendicular approaches can never share right-of-way). Greedy graph
        coloring partitions the 4 lanes into 2 mutually non-conflicting phase groups (NORTH+SOUTH,
        EAST+WEST) — the same abstraction real traffic-signal controllers use to design safe simultaneous phases.
      </p>
    </div>
  );
};

const VehicleHashLookup = ({ laneData }) => {
  const [query, setQuery] = useState('');

  // DSA: Hash Map. Build a hash map (JS Map, O(1) average get/set) keyed by
  // vehicle id from all lanes, rebuilt whenever laneData changes. Used below
  // for O(1) point lookups instead of scanning every lane's queue.
  const vehicleIndex = useMemo(() => {
    const map = new Map();
    Object.keys(laneData).forEach((lane) => {
      laneData[lane].forEach((car) => {
        map.set(car.id, { ...car, lane });
      });
    });
    return map;
  }, [laneData]);
  ...
  const result = trimmed ? (vehicleIndex.get(trimmed) || null) : null;

  return (
    <div className="bg-white border border-[#D1D5DB] rounded-2xl p-4 shadow-sm text-xs font-mono text-[#111827] space-y-3">
      <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-2">
        <h3 className="font-bold text-xs uppercase tracking-wider text-[#7C3AED]">
          Vehicle Registry (Hash Map Lookup)
        </h3>
        <span className="text-[9px] text-[#6B7280]">{vehicleIndex.size} indexed</span>
      </div>
      ...
      <input
        ...
        placeholder="Enter Process ID e.g. N-427"
        className="w-full px-2.5 py-1.5 bg-[#F9FAFB] border border-[#D1D5DB] rounded-lg text-[11px] font-mono outline-none focus:border-[#7C3AED]"
      />

      {trimmed && (
        result ? (
          <div className="bg-[#F5F3FF] border border-[#DDD6FE] rounded-lg p-2 space-y-0.5 text-[11px]">
            <div className="font-bold text-[#6D28D9]">{result.id} — found in O(1)</div>
            <div className="text-[#4B5563]">Lane: {result.lane} · Type: {result.type} · Priority: P{result.priority}</div>
            <div className="text-[#4B5563]">Burst: {result.burstTime}s · Wait: {result.waitTime}s</div>
          </div>
        ) : (
          <div className="text-[11px] text-[#9CA3AF] italic">No vehicle with that ID currently queued.</div>
        )
      )}

      <p className="text-[10px] text-[#6B7280] leading-relaxed font-sans">
        Every queued vehicle is indexed by ID in a hash map, giving O(1) average-case lookup instead of an O(n)
        linear scan across all 4 lane queues.
      </p>
    </div>
  );
};

const CriticalSectionMonitor = ({ activeLane, isYellow, laneData }) => {
  const locked = !isYellow;
  const holder = isYellow ? null : activeLane;
  const order = ['NORTH', 'EAST', 'SOUTH', 'WEST'];
  const waitingOnLock = order.filter(
    (lane) => lane !== holder && laneData[lane] && laneData[lane].length > 0
  );

  return (
    <div className="bg-white border border-[#D1D5DB] rounded-2xl p-4 shadow-sm text-xs font-mono text-[#111827] space-y-3">
      <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-2">
        <h3 className="font-bold text-xs uppercase tracking-wider text-[#0F766E]">
        ...
        >
          S = {locked ? '0 (LOCKED)' : '1 (FREE)'}
        </span>
      </div>

      <div className="grid grid-cols-2 gap-2">
        <div className="bg-[#F9FAFB] p-2 rounded-lg border border-[#E5E7EB]">
          <div className="text-[#6B7280]">CRITICAL SECTION</div>
          <div className="font-bold text-[#111827] mt-0.5">Intersection Center</div>
        </div>
        <div className="bg-[#F9FAFB] p-2 rounded-lg border border-[#E5E7EB]">
          <div className="text-[#6B7280]">LOCK HELD BY</div>
          <div className="font-bold text-[#0F766E] mt-0.5">
            {holder || 'None (clearance phase)'}
          </div>
        </div>
      </div>

      <div className="bg-[#F9FAFB] p-2 rounded-lg border border-[#E5E7EB]">
        <div className="text-[#6B7280] mb-1">BLOCKED ON wait(S) — NOT ADMITTED TO CS</div>
        {waitingOnLock.length === 0 ? (
          <div className="text-[11px] text-[#9CA3AF] italic">No queue currently blocked on the semaphore.</div>
        ) : (
          <div className="flex flex-wrap gap-1.5">
            {waitingOnLock.map((lane) => (
              <span
                key={lane}
                className="px-1.5 py-0.5 bg-white border border-[#E5E7EB] rounded text-[10px] font-bold text-[#4B5563]"
              >
                {lane}
              </span>
            ))}
      ...
      <p className="text-[10px] text-[#6B7280] leading-relaxed font-sans">
        <span className="font-bold text-[#374151]">Mutual exclusion:</span> the intersection center is a shared
        resource. A binary semaphore <code>S</code> gates entry — a lane must execute <code>wait(S)</code> (acquire,
        S → 0) before crossing, and <code>signal(S)</code> (release, S → 1) during the yellow clearance phase before
        the next lane can acquire it. Exactly one lane occupies the critical section at any instant.
      </p>
    </div>
  );
};

const LaneCapacityPanel = ({ laneData }) => {
  const order = ['NORTH', 'EAST', 'SOUTH', 'WEST'];

  return (
    <div className="bg-white border border-[#D1D5DB] rounded-2xl p-4 shadow-sm text-xs font-mono text-[#111827] space-y-3">
      <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-2">
        <h3 className="font-bold text-xs uppercase tracking-wider text-[#4338CA]">
          Memory Management — Bounded Buffers
        </h3>
        <span className="text-[9px] text-[#6B7280]">Capacity N = {LANE_CAPACITY}</span>
      </div>

      <div className="space-y-2">
        {order.map((lane) => {
          const occupied = laneData[lane]?.length || 0;
          const ratio = occupied / LANE_CAPACITY;
          const barColor = ratio >= 1 ? 'bg-rose-500' : ratio >= 0.7 ? 'bg-amber-500' : 'bg-emerald-500';
          return (
            <div key={lane} className="space-y-0.5">
              <div className="flex items-center justify-between text-[10px]">
                <span className="font-bold text-[#374151]">{lane}</span>
                <span className={occupied >= LANE_CAPACITY ? 'text-rose-600 font-bold' : 'text-[#6B7280]'}>
                  {occupied}/{LANE_CAPACITY} slots{occupied >= LANE_CAPACITY ? ' — FULL' : ''}
                </span>
              </div>
      ...
      <p className="text-[10px] text-[#6B7280] leading-relaxed font-sans">
        <span className="font-bold text-[#374151]">Producer-Consumer:</span> "Inject Process Thread" is the producer
        — it inserts into the buffer and blocks once a lane hits {LANE_CAPACITY}/{LANE_CAPACITY}. The scheduler
        completing a CPU burst is the consumer — it removes from the buffer and frees a slot.
      </p>
    </div>
  );
};
...
const DeadlockLab = ({
  ...
  const edges = Object.entries(deadlockWaitFor || {});

  const statusLabel = deadlockRecovered
    ? 'RECOVERED'
    : deadlockDetected
    ? 'DEADLOCK CONFIRMED'
    : deadlockActive
    ? 'CIRCULAR WAIT ACTIVE'
    : 'IDLE — NO DEADLOCK';

  const statusColor = deadlockRecovered
    ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
    : deadlockDetected
  ...
  return (
    <div className="bg-white border border-[#D1D5DB] rounded-2xl p-4 shadow-sm text-xs font-mono text-[#111827] space-y-3">
      <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-2">
        <h3 className="font-bold text-xs uppercase tracking-wider text-[#B91C1C]">Deadlock Lab</h3>
      ...
      <div className="flex justify-center">
        <svg viewBox="0 0 280 200" className="w-full max-w-[220px]">
          {edges.map(([from, to]) => {
            const isInCycle = deadlockCycle.includes(from) && deadlockCycle.includes(to);
            ...
            return (
              <g key={`${from}-${to}`}>
                <line
                  x1={p1.x + 20} y1={p1.y + 20}
                  x2={p2.x + 20} y2={p2.y + 20}
                  stroke={isInCycle ? '#DC2626' : '#94A3B8'}
                  strokeWidth={isInCycle ? 2.5 : 1.5}
                  markerEnd="url(#arrowhead)"
                />
                <text x={midX + 18} y={midY + 14} fontSize="8" fill="#6B7280" fontFamily="monospace">
                  wants
                </text>
              </g>
            );
          ...
          {Object.entries(positions).map(([lane, pos]) => (
            <g key={lane}>
              <circle
                cx={pos.x + 20} cy={pos.y + 20} r="20"
                fill={deadlockCycle.includes(lane) ? '#FEE2E2' : '#F8FAFC'}
                stroke={deadlockCycle.includes(lane) ? '#DC2626' : '#CBD5E1'}
                strokeWidth="2"
              />
              <text x={pos.x + 20} y={pos.y + 24} fontSize="9" fontWeight="bold" textAnchor="middle" fill="#111827">
                {lane.slice(0, 1)}
      ...
      <div className="flex flex-wrap gap-1.5">
        <button
          onClick={onTrigger}
          disabled={deadlockActive}
          className="px-2 py-1 bg-rose-50 hover:bg-rose-100 disabled:opacity-40 disabled:cursor-not-allowed text-rose-700 border border-rose-200 rounded-lg font-bold text-[10px]"
        >
          Trigger Deadlock
        </button>
        <button
          onClick={onDetect}
          disabled={!deadlockActive || deadlockDetected}
          className="px-2 py-1 bg-amber-50 hover:bg-amber-100 disabled:opacity-40 disabled:cursor-not-allowed text-amber-700 border border-amber-200 rounded-lg font-bold text-[10px]"
        >
          Detect (DFS Cycle Check)
        </button>
        <button
          onClick={onRecover}
          disabled={!deadlockDetected}
          className="px-2 py-1 bg-emerald-50 hover:bg-emerald-100 disabled:opacity-40 disabled:cursor-not-allowed text-emerald-700 border border-emerald-200 rounded-lg font-bold text-[10px]"
        >
          Recover (Preempt)
        </button>
        {deadlockActive && (
          <button
            onClick={onExit}
            className="px-2 py-1 bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 rounded-lg font-bold text-[10px]"
          >
            Exit Lab / Resume
          </button>
        )}
      ...
      <p className="text-[10px] text-[#6B7280] leading-relaxed font-sans">
        Each lane holds its own quadrant lock and requests the next lane's lock (hold-and-wait), no lock can be
        force-taken (no preemption), and the request chain forms a cycle N → E → S → W → N (circular wait) — with
        the intersection's inherent mutual exclusion, all four Coffman conditions hold. Detection runs a DFS cycle
        check over this wait-for graph; recovery force-preempts one lane's lock to break the cycle.
      </p>
    </div>
  );
};
    ...
    <div className="bg-white border border-[#D1D5DB] rounded-2xl p-4 space-y-3 shadow-sm font-sans text-[#111827]">
      ...
      <div className="grid grid-cols-3 gap-2 text-[10px] font-mono text-center pt-1">
        <div className="bg-[#F9FAFB] p-2 rounded-lg border border-[#E5E7EB]">
          ...
          <div className="font-bold text-[#0284C7] mt-0.5">
            {ALGORITHM_OPTIONS.find((a) => a.id === selectedAlgo)?.name.split(' (')[0] || selectedAlgo}
          </div>
        </div>
        <div className="bg-[#F9FAFB] p-2 rounded-lg border border-[#E5E7EB]">
          <div className="text-[#6B7280]">DISPATCH MODE</div>
          <div className="font-bold text-[#15803D] mt-0.5">
            {selectedAlgo === 'RR' ? `Preemptive (${timeQuantum}s)` : selectedAlgo === 'PRIORITY_AGING' ? 'Preemptive' : 'Non-Preemptive'}
          </div>
        </div>
        <div className="bg-[#F9FAFB] p-2 rounded-lg border border-[#E5E7EB]">
          <div className="text-[#6B7280]">DYNAMIC AGING</div>
          <div className={`font-bold mt-0.5 ${selectedAlgo !== 'PRIORITY_AGING' ? 'text-[#9CA3AF]' : agingEnabled ? 'text-[#15803D]' : 'text-[#EF4444]'}`}>
            {selectedAlgo !== 'PRIORITY_AGING' ? 'N/A' : agingEnabled ? 'Enabled' : 'Disabled'}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-white border border-[#D1D5DB] p-4 rounded-2xl flex flex-wrap items-center justify-between gap-4 shadow-sm text-[#111827] font-mono">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {(selectedAlgo === 'FCFS' || selectedAlgo === 'SJF') && (
          <div className="flex items-center space-x-2 bg-[#F8F9FA] border border-[#D1D5DB] px-3 py-1.5 rounded-xl text-xs font-sans">
            <span className="text-[#4B5563] font-mono text-[11px]">
              {selectedAlgo === 'FCFS' ? 'Order key: arrival time' : 'Order key: burst time'}
            </span>
            <span className="text-[10px] font-bold text-[#6B7280]">(non-preemptive, waits for burst to finish)</span>
          </div>
        )}
      </div>
...
const SimulationDashboard = ({ onBackToLanding }) => {
  ...
  // --- Deadlock Lab state: a separate, explicit demo of circular wait,
  // detection (via wait-for graph cycle detection) and recovery.
  const [deadlockActive, setDeadlockActive] = useState(false);
  const [deadlockDetected, setDeadlockDetected] = useState(false);
  const [deadlockWaitFor, setDeadlockWaitFor] = useState({});
  const [deadlockCycle, setDeadlockCycle] = useState([]);
  const [deadlockRecovered, setDeadlockRecovered] = useState(false);
  ...
  const addVehicle = (laneKey, typeOverride = null) => {
    const currentOccupancy = laneData[laneKey].length;

    if (currentOccupancy >= LANE_CAPACITY) {
      addLog('MEMORY', `Producer BLOCKED: ${laneKey} bounded buffer full (${currentOccupancy}/${LANE_CAPACITY} slots). Producer must wait for the consumer (dispatch completion) to free a slot before inserting.`);
      return;
    }
    ...
    addLog('MEMORY', `Producer inserted ${newVehicle.id} into ${laneKey} bounded buffer (${currentOccupancy + 1}/${LANE_CAPACITY} slots occupied).`);
  ...
  // --- Deadlock Lab: models each lane as holding its own quadrant lock and
  // requesting the next lane's lock, i.e. a functional wait-for graph
  // NORTH -> EAST -> SOUTH -> WEST -> NORTH, which by construction satisfies
  // all four Coffman conditions (mutual exclusion, hold-and-wait, no
  // preemption, circular wait).
  const triggerDeadlock = () => {
    setIsRunning(false);
    setDeadlockActive(true);
    setDeadlockDetected(false);
    setDeadlockRecovered(false);
    setDeadlockCycle([]);
    setDeadlockWaitFor({ NORTH: 'EAST', EAST: 'SOUTH', SOUTH: 'WEST', WEST: 'NORTH' });
    addLog('DEADLOCK', 'Deadlock scenario triggered: each lane holds its own quadrant lock and requests the next lane\'s lock -> circular wait. Simulation clock frozen pending detection & recovery.');
  };

  // Generic cycle detection over a wait-for graph via DFS with a recursion
  // stack (standard directed-graph cycle detection), not a hardcoded result.
  const detectDeadlock = () => {
    const graph = deadlockWaitFor;
    const visited = {};
    const onStack = {};
    let foundCycle = null;
    ...
    if (foundCycle) {
      ...
      setDeadlockCycle(foundCycle);
      addLog('DEADLOCK', `Deadlock CONFIRMED — DFS cycle detection on the wait-for graph found: ${foundCycle.join(' -> ')} -> ${foundCycle[0]}. Circular wait verified.`);
    } else {
      addLog('DEADLOCK', 'Cycle detection ran over the wait-for graph: no cycle found.');
    }
  };

  const recoverFromDeadlock = () => {
    if (deadlockCycle.length === 0) return;
    const victim = deadlockCycle[0];

    addLog('DEADLOCK', `Recovery: controller force-preempts the lock held by ${victim} (resource preemption), breaking the circular wait.`);
    addLog('SEMAPHORE', `signal(S) forced by controller on behalf of ${victim} -> lock released -> normal dispatch may resume.`);

    setDeadlockRecovered(true);
    setDeadlockDetected(false);
    setDeadlockWaitFor({});
  };

  const exitDeadlockLab = () => {
  ...
  const advanceSimulationStep = () => {
    if (deadlockActive) return; // frozen while the deadlock scenario is being inspected/recovered
    ...
    setLaneData((prevData) => {
      ...
      if (activeQueue.length > 0) {
        ...
        if (currentCar.burstTime <= 0) {
          ...
          addLog('MEMORY', `Consumer freed 1 slot in ${activeLane} bounded buffer (${activeQueue.length}/${LANE_CAPACITY} slots now occupied).`);
        }
      }
    ...
    if (selectedAlgo === 'RR') {
      ...
      if (targetLane !== activeLane) {
        ...
        addLog('SEMAPHORE', `signal(S) by ${activeLane} — releases critical section lock. wait(S) by ${targetLane} pending until clearance ends.`);
      }
    } else if (selectedAlgo === 'FCFS' || selectedAlgo === 'SJF') {
      // Non-preemptive: only re-dispatch once the running process finishes its burst
      // (or the active lane has nothing running), never mid-burst.
      const currentActiveQueue = laneData[activeLane];
      const currentCarFinishing = currentActiveQueue.length > 0 && (currentActiveQueue[0].burstTime - 1) <= 0;
      const activeQueueEmpty = currentActiveQueue.length === 0;

      if (currentCarFinishing || activeQueueEmpty) {
        const order = ['NORTH', 'EAST', 'SOUTH', 'WEST'];
        let bestLane = null;
        ...
        order.forEach((lane) => {
          const queue = lane === activeLane && currentCarFinishing
            ? laneData[lane].slice(1)
            : laneData[lane];
          if (queue.length > 0) {
            const topCar = queue[0];
            const value = selectedAlgo === 'FCFS' ? topCar.arrivalTime : topCar.burstTime;
            if (value < bestValue) {
              bestValue = value;
              bestLane = lane;
              bestCarId = topCar.id;
        ...
        if (bestLane && bestLane !== activeLane) {
          setIsYellow(true);
          setContextSwitchCount((c) => c + 1);
          ...
          addLog('SEMAPHORE', `signal(S) by ${activeLane} — releases critical section lock. wait(S) by ${bestLane} pending until clearance ends.`);
        }
      }
    }
  };
      ...
      {viewMode === 'COMPARE' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
          ...
          <div className="space-y-4">
            ...
            <ConflictGraphPanel activeLane={activeLane} />

            <VehicleHashLookup laneData={laneData} />

            <LaneCapacityPanel laneData={laneData} />

            <DeadlockLab
              deadlockActive={deadlockActive}
              deadlockDetected={deadlockDetected}
              deadlockRecovered={deadlockRecovered}
              deadlockWaitFor={deadlockWaitFor}
              deadlockCycle={deadlockCycle}
              onTrigger={triggerDeadlock}
              onDetect={detectDeadlock}
              onRecover={recoverFromDeadlock}
              onExit={exitDeadlockLab}
            />
